  ...
  return (
    ...
    >
        ...
        {cartItems.map((item) => (
          <Box key={item.id} sx={{ display: "flex", gap: 2, mb: 2 }}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Typography variant="body2" sx={{color: colors["--clr-gray-5"]}}>Qtd: {item.quantity}</Typography>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        ...
        <Typography variant="h6" sx={{ fontWeight: 700, color: colors["--clr-gray-2"] }}>$ {total.toFixed(2)}</Typography>
      </Box>
    </Paper>
  );
}
